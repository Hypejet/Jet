...
public final class HolderBinaryTagCodec<V> implements BinaryTagCodec<Holder<V>> {
    ...
    @Override
    public Holder<V> decode(BinaryTag binaryTag) {
        try {
            return new Holder.Reference<>(KeyBinaryTagCodec.INSTANCE.decode(binaryTag));
        } catch (Exception exception) {
            if (!this.allowInline)
                throw new IllegalArgumentException("Inline definitions are not allowed here");
            return new Holder.Direct<>(this.valueCodecOrThrow().decode(binaryTag));
        }
    }

    @Override
    public BinaryTag encode(Holder<V> value) {
        return switch (value) {
            ...
            default -> throw new IllegalArgumentException("Unknown holder: " + value);
        };
    }
