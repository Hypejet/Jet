...
public final class HolderSetBinaryTagCodec<V> implements BinaryTagCodec<HolderSet<V>> {
    ...
    @Override
    public HolderSet<V> decode(BinaryTag binaryTag) {
        try {
            return new HolderSet.Named<>(KeyBinaryTagCodec.HASHED_INSTANCE.decode(binaryTag));
        } catch (Exception exception) {
            List<Holder<V>> contents = this.holderListCodec.decode(binaryTag);
            return new HolderSet.Direct<>(validateList(contents));
        }
    }

    @Override
    public BinaryTag encode(HolderSet<V> value) {
        return switch (value) {
            ...
            default -> throw new IllegalStateException(
                    "Unknown holder set class: " + value.getClass().getSimpleName()
            );
        };
    }
